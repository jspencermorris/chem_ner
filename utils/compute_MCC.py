...
def compute_MCC_jsonl(references_jsonl, predictions_jsonl, id_col, ref_col='ner_tags', pred_col='pred_ner_tags', ignore_label='O'):
    ...
    Additionally, computes MCC with and without the 'ignore_label'.
    '''
    # Reverse the dict
    ...
    # Compute MCC for all labels
    mcc_score = matthews_corrcoef(y_true=flat_ref_tags, y_pred=flat_pred_tags)

    # Mask out the 'ignore_label' and compute MCC again
    mask = (flat_ref_tags != ignore_label) & (flat_pred_tags != ignore_label)
    filtered_ref_tags = flat_ref_tags[mask]
    filtered_pred_tags = flat_pred_tags[mask]
    mcc_score_no_other = matthews_corrcoef(y_true=filtered_ref_tags, y_pred=filtered_pred_tags)
    
    return mcc_score, mcc_score_no_other
